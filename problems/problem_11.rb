...
def up_to_down(row, index)
	...
	0.upto(row.length-1) do |i|
		product  = row[i].to_i
		product *= eval("@row#{index+1}[i]").to_i
		product *= eval("@row#{index+2}[i]").to_i
...
def left_to_right_diagonal(row, index)
	...
	0.upto(row.length-4) do |i|
		product  = row[i].to_i
		product *= eval("@row#{index+1}[i+1]").to_i
		product *= eval("@row#{index+2}[i+2]").to_i
...
1.upto(20) {|i| eval("nums << left_to_right(@row#{i})")}
1.upto(17) {|i| eval("nums << up_to_down(@row#{i}, #{i})")}
1.upto(17) {|i| eval("nums << left_to_right_diagonal(@row#{i}, #{i})")}
1.upto(17) {|i| eval("nums << right_to_left_diagonal(@row#{i}, #{i})")}
